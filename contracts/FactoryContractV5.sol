    ...
    AccessControlUpgradeable
{
    ...
    event AddNumber(uint256 x, address addr);
    ...
    */
    ...
    function transferToken(
        ...
        returns (bool)
    {
        validTypes.push(_value);
        for (uint i; i < validTypes.length; i++) {
            if (_type != validTypes[i]) {
                return false;
            }
        }
    ...
    function addNumber(uint256 _x) public onlyAdmins {
        add = _x;

        emit AddNumber(_x, _msgSender());
    }
